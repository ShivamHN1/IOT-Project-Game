/* autogenerated by Processing revision 1293 on 2026-04-22 */
 ...
 * ============================================================================
 */
  // Handles Arduino/Joystick communication
   // Handles procedural audio generation
 ...
 */
public void setup() {
  /* size commented out by preprocessor */;  // Defines the application window resolution
...
public void draw() {
  ...
  if (gameState == 0) {
    ...
    fill(255, 255, 100, map(sin(frameCount * 0.1f), -1, 1, 50, 255)); // Glowing yellow
    textSize(24);
    text("PRESS SPACE TO START", width / 2, height / 2 + 80);
    
    // Aesthetic decorative ship at bottom
    pushMatrix();
    translate(width / 2, height - 100 + sin(frameCount * 0.05f) * 10); // Hover effect
    noStroke();
  ...
  if (screenShake > 0) {
    ...
    screenShake *= 0.9f; // Decay the shake rapidly
  ...
  if (!useSerial) {
    ...
    float accel = 0.8f;
    float maxSpeed = 6.0f;
    float friction = 0.85f; // Sliding friction
    ...
    if (spaceHeld && shootCooldown <= 0) {
      ...
      playerLaser.play();
      envPLaser.play(playerLaser, 0.01f, 0.05f, 0.1f, 0.1f);
  ...
  float targetBank = map(playerVX, -6, 6, -0.4f, 0.4f);
  playerBank = lerp(playerBank, targetBank, 0.2f);
  ...
  for (int i = bullets.size() - 1; i >= 0; i--) {
    ...
    if (b.isOffScreen()) {
      ...
      hurtSound.play();
      envHurt.play(hurtSound, 0.01f, 0.1f, 0.5f, 0.2f); // Louder and sharper than explosion
  ...
  for (int i = enemies.size() - 1; i >= 0; i--) {
    ...
    if (invincibilityTimer <= 0 && dist(playerX, playerY, e.x, e.y) < (playerSize + e.size) / 2) {
      ...
      hurtSound.play();
      envHurt.play(hurtSound, 0.01f, 0.1f, 0.6f, 0.3f);
...
public void serialEvent(Serial myPort) {
  ...
  if (values.length == 3) {
    try {
      float parsedX = PApplet.parseFloat(values[0]);
      float parsedY = PApplet.parseFloat(values[1]);
      int buttonState = PApplet.parseInt(values[2]);
 ...
 */
public void createExplosion(float ex, float ey) {
  // Explosion sound
  explosionSound.freq(random(80, 120));
  explosionSound.play();
  envExplosion.play(explosionSound, 0.01f, 0.1f, 0.3f, 0.2f);
...
class Enemy {
  ...
  Enemy() {
    ...
    speed = 1.0f + (level * 0.2f); 
    shootOffset = PApplet.parseInt(random(0, 60)); // Randomize shoot timing
  }
  ...
  public void update() {
    ...
    if (y > 20 && (frameCount + shootOffset) % shootChance == 0) {  
      ...
      enemyLaser.play();
      envELaser.play(enemyLaser, 0.01f, 0.05f, 0.05f, 0.1f);
    }
  }
  ...
  public void show() {
    ...
    ellipse(x, y, size * 0.8f, size);
    
    // Enemy wings
    fill(150, 30, 30);
    triangle(x - size * 0.4f, y, x - size * 0.8f, y - size * 0.4f, x - size * 0.4f, y + size * 0.3f);
    triangle(x + size * 0.4f, y, x + size * 0.8f, y - size * 0.4f, x + size * 0.4f, y + size * 0.3f);

    // Enemy Cockpit
    fill(50, 255, 50); // Greenish alien window
    ellipse(x, y + 5, size * 0.4f, size * 0.2f);
  }
...
class Bullet {
  ...
  public void show() {
    ...
    // Glowing laser body
    fill(255, 0, 0, 60);
    rect(x, y, size + 12, size * 3.5f, 10); // Big faint aura
    fill(255, 50, 50, 150);
    rect(x, y, size + 4, size * 2.5f, 5);   // Outer glow
    fill(255, 255, 150);
    rect(x, y, size - 2, size * 1.5f, 5);   // Bright hot core
  }
  ...
  public boolean hitsPlayer(float px, float py, float pSize) {
    // Shrank the hitbox so players can barely dodge bullets
    return dist(x, y, px, py) < (size + pSize * 0.3f) / 2;
  }
}
...
class PlayerBullet {
  ...
  public boolean hitsEnemy(Enemy e) {
    // Shrank hitbox slightly for tighter aiming requirement
    return dist(x, y, e.x, e.y) < (size + e.size * 0.7f) / 2;
  }
}
...
class Star {
  ...
  Star() {
    ...
    speed = size * 0.8f;       // Parallax: bigger stars move faster
    
    // Give stars subtle color variation matching real space
    float r = random(1);
    if (r < 0.2f) starColor = color(200, 220, 255);      // Hot blue
    else if (r < 0.3f) starColor = color(255, 220, 200); // Red giant
    else if (r < 0.4f) starColor = color(255, 255, 200); // Yellow
  ...
  public void update() {
    ...
    if (y > height) {
      ...
      float r = random(1);
      if (r < 0.2f) starColor = color(200, 220, 255);
      else if (r < 0.3f) starColor = color(255, 220, 200);
      else if (r < 0.4f) starColor = color(255, 255, 200);
...
class Particle {
  ...
  Particle(float startX, float startY, boolean flash) {
    ...
    if (isFlash) {
      ...
      float r = random(1);
      if (r < 0.3f)      c = color(255, 200, 0); // Yellow
      else if (r < 0.7f) c = color(255, 100, 0); // Orange
  ...
  public void show() {
    ...
    fill(red(c), green(c), blue(c), life * 0.3f);
    ellipse(x, y, pSize * 2.5f, pSize * 2.5f);
    ...
    if (isFlash) {
      ...
      ellipse(x, y, pSize * 0.5f, pSize * 0.5f);
    }
  }
...
class FloatingText {
  ...
  public void update() {
    y -= 1.0f; // Float upwards slowly
    textLife -= 5.0f; // Fade out gradually
  }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ShooterGame" };
    if (passedArgs != null) {
